...
import { type FormMessageType } from '../../../types/types'
import validate from './validates'
import SpinLoader from '../../../assets/componentSVG/SpinLoader'
...
const FormContact = () => {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col items-center gap-8 w-full">
      ...
      <p
      className={`${messageAl !== '' ? '' : 'hidden'} font-noto text-base text-center text-red-700`}>
        {messageAl === 'OK' ? '*Llena todos los campos' : '*Corrige los Errores'}
      </p>

      <p
      className={`${response !== '' ? '' : 'hidden'} font-noto text-base text-center ${response === 'OK' ? 'text-green-600' : 'text-red-700'}`}>
